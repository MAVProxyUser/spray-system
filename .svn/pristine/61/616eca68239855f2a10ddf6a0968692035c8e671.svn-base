...
//add 2010-08-09
extern U8 usb_on_flag;
extern unsigned short  volatile CDC_DepInEmpty;
/*
 *  Reset USB Core
 *    Parameters:      None
 *    Return Value:    None
...
#endif  /* USB_HID */
#if USB_MSC
              //modify 2010-12-16 必须改为此变量var_USB_MSC_IF_NUM
			  //if (SetupPacket.wIndex.WB.L == USB_MSC_IF_NUM) {               /* IF number correct? */
                if (SetupPacket.wIndex.WB.L == var_USB_MSC_IF_NUM) {           /* IF number correct? */
				switch (SetupPacket.bRequest) {
...
setup_class_ok:
		  ...
		  usb_on_flag = 1;
		  //add 2010-08-20 
		  CDC_DepInEmpty  = 1;                                             /* request finished successfully */
