...
#define CCLKCFG_Val           0x00000002     //3分频？
#define USBCLKCFG_Val         0x00000000
#define PCLKSEL0_Val          0x00000000
#define PCLKSEL1_Val          0x00000000
#define PCONP_Val             0x8000E25E
...
void SystemInit (void)
{
  ...
  LPC_SC->CCLKCFG   = CCLKCFG_Val;      /* Setup Clock Divider                */ //CPU时钟3分频？

  LPC_SC->PCLKSEL0  = PCLKSEL0_Val;     /* Peripheral Clock Selection         *///外设时钟1/4分频  
...
#if (PLL0_SETUP)
  LPC_SC->CLKSRCSEL = CLKSRCSEL_Val;    /* Select Clock Source for PLL0       */ //选择PLL0时钟源    01-主振荡器
  LPC_SC->PLL0CFG   = PLL0CFG_Val;                                               //分频值和倍频值  
  LPC_SC->PLL0CON   = 0x01;             /* PLL0 Enable                        */
  LPC_SC->PLL0FEED  = 0xAA;
  LPC_SC->PLL0FEED  = 0x55;
  while (!(LPC_SC->PLL0STAT & (1<<26)));/* Wait for PLOCK0                    */  //PLL0锁定指定频率    
  ...
  LPC_SC->PLL1FEED  = 0x55;
#else
  LPC_SC->USBCLKCFG = USBCLKCFG_Val;    /* Setup USB Clock Divider            */  //仅PLL0 USB可分频   
#endif
  ...
  if (((LPC_SC->PLL0STAT >> 24)&3)==3) {/* If PLL0 enabled and connected      */
    ...
    LPC_SC->PCONP   |= (1UL<<15);       /* enable gpio       */    
}
