...
spray_system_flow_cali_para_t spray_system_flow_cali_para1 __attribute__((aligned(4))); //Á÷Á¿¼Æ²ÎÊı
...
/************************************Á÷Á¿´«¸ĞÆ÷Êı¾İ´¦Àí*************************************************************/
extern int16_t flow_sensor_value; // for test ºóĞøĞèÒªÓÅ»¯
...
float Cali_Freq ; // Ğ£×¼ÍêÊ±µÄÆ½¾ùÂö³åÊı
float debug_watch_k_value = 0;
float Viscosity_Offset_Coeff = 0.05;// Õ³¶È²îÏµÊı
#define P_FREQ       0.6
#define Reference_K  0.49  // ÇåË®²â³öÀ´µÄÄ¬ÈÏKÖµ
// ¸ù¾İÕ³¶ÈµÄ²»Í¬¶Ô²»Í¬Á÷ËÙÏÂµÄKÖµ½øĞĞ²¹³¥
//float debug_smm;
float Viscosity_Offset = 0,K_offset = 0;
float K_Coefficient_Compensation(void) // Á÷Á¿¼ÆKÖµÏµÊı²¹³¥
{

// vk04  ±ê¶¨Á÷Á¿  0.9L/min---1.2L/min      Õ³¶È²î²¹³¥ÏµÊı               ÇåË®KÖµ 0.5 
// 11001 ±ê¶¨Á÷Á¿  1.4L/min---1.8L/min      Õ³¶È²î²¹³¥ÏµÊı  0.5
// vk08 ±ê¶¨Á÷Á¿   1.95L/min---2.2L/min     Õ³¶È²î²¹³¥ÏµÊı  0.8
// Óë±ê¶¨ÆµÂÊµÄÂö³å²î ±íÊ¾Á÷ËÙ±ä»¯
// ¼´×îÖÕ¹«Ê½Îª  Âö³åÆµÂÊ*K(1+Õ³¶È²î*0.5*(±ê¶¨ÆµÂÊ - ÊµÊ±ÆµÂÊ))

     if((Volume_Each_Pulse) - Reference_K > 0)
		 {			 
				// ÅĞ¶ÏÎªÕ³³íÒºÌå  ¼õÈ¥Æ«ÒÆÖµ
				Viscosity_Offset  =	((Volume_Each_Pulse) - Reference_K)*Viscosity_Offset_Coeff;
		 }
		 else
		 {
		   Viscosity_Offset = 0; // ÅĞ¶ÏÎªÇåË®
		 }
//	}
...
{
	if(current_capacity_part > 25500)//16Î» ¼ÆÊı¼Ó´ó »á´¥·¢µ¥×Ö½ÚÒç³ö
...
{
  Volume_Each_Pulse = value ; // + 0.03; // ÎªÁË·ÀÖ¹Îó²îµ¼ÖÂÒ©Á¿²»¹»ÓÃ
}

void capacity_by_flow_sensor(void)// Ö»ÄÜÓĞÒ»¸öµØ·½ÔËĞĞ
{	
		if(get_cali_flow_freq() > 5) // ÔİÊ±´¦Àí·½Ê½ Ğ¡ÓÚÕâ¸öÆµÂÊÄ¬ÈÏÎª0
		{
			 debug_watch_k_value = Volume_Each_Pulse+K_Coefficient_Compensation();
			 flow_compute = get_cali_flow_freq()*Volume_Each_Pulse*(1+K_Coefficient_Compensation()); // Ã¿Ãë¶àÉÙml
				...
				// Èç¹ûKÖµ²»¶Ô£¬ÔÚ±ä»¯·¶Î§ÖĞÓÃĞ£×¼µÄÓÍÃÅÖµ×öÏŞÖÆ
				 ...
				 { 
				  target_pules = (float)((1000*foc_status.flow_speed *0.02f)/(Volume_Each_Pulse*60))+0.5f;//ËÄÉáÎåÈë 
					...
					if((fabs(target_pules - get_cali_flow_freq())+0.5f) > 1 )//Èç¹ûÂö³åÊı²îÌ«¶àÔòÍ£Ö¹Ğ£×¼
		  ...
		  if(last_flow_1 == 0)
			...
			if(control_freq_1 % 5 == 0 ) // 1hz
			{
				// Èç¹ûKÖµ²»¶Ô£¬ÔÚ±ä»¯·¶Î§ÖĞÓÃĞ£×¼µÄÓÍÃÅÖµ×öÏŞÖÆ
				 ...
				 if(last_status_1 == 1)
				 {
					if((fabs(target_pules_1 - get_cali_flow_freq())+0.5f) > 1 )//Èç¹ûÂö³åÊı²îÌ«¶àÔòÍ£Ö¹Ğ£×¼
		  ...
		  if(last_flow_2 == 0)
			...
			if(control_freq_2 % 5 == 0 ) // 1 hz
			{
				// Èç¹ûKÖµ²»¶Ô£¬ÔÚ±ä»¯·¶Î§ÖĞÓÃĞ£×¼µÄÓÍÃÅÖµ×öÏŞÖÆ
				 ...
				 if(last_status_2 == 1)
				 {
					if((fabs(target_pules_2 - get_cali_flow_freq())+0.5f) > 1 )//Èç¹ûÂö³åÊı²îÌ«¶àÔòÍ£Ö¹Ğ£×¼
...
void Control_The_Frequency_Pulse_1(uint16_t foc_value,uint8_t target_value)
{
	 ...
	 if(abs(target_value - get_cali_flow_freq()) > 2 && (get_cali_flow_freq() > (target_value - 8)))//Èç¹ûÂö³åÊı²îÌ«¶àÔòÍ£Ö¹Ğ£×¼
...
{
  spray_system_flow_cali_para1.capacity = value - 600;// 600ml Ä¬ÈÏ´¥·¢ÒºÎ»¿ª¹Ø
}
	   ...
	   case 0:
		 ...
		 case 1:
          ...
          // È¡Õâ¸öÊ±ºòµÄÂö³åÊıÎªÄ¿±êÆµÂÊÒ»¶ÎÊ±¼ä
				 if(switch_count >= 80) //ÎÈ¶¨ ±£Ö¤Á÷Á¿¼ÆÊıÎÈ¶¨
				 {
					 switch_count = 80;
		 ...
		 case 1:
		   ...
		   if(flow_cali_value == MIN_CNT)
			 ...
			 else if(flow_cali_value > MIN_CNT)
			 {
				 ...
				 else          // È¡Õâ¸öÊ±ºòµÄÂö³åÊıÎªÄ¿±êÆµÂÊ  //ÎÈ¶¨Á½Ãë ±£Ö¤Á÷Á¿¼ÆÊıÎÈ¶¨
				 {
     ...
     break;			 
		 ...
		 case 2:	
		   ...
		   // Èç¹ûÊ¹ÄÜÅçÈ÷Ö´ĞĞÕâ
			    ...
			    if(freq_control_count % 20 == 0)
				  {
						  Control_The_Frequency_Pulse_2(flow_cali_value,Med_Frequency_Pulse);// ÉèÖÃÎªÂö³åÊı1µÄÄ¿±êÂö³å
...
uint8_t MedeRunoutFlag = 0; // ³õÊ¼»¯ÎªÓĞÒ©
uint8_t remaining_capacity_detection( void )// 100HZ 
{
			 ...
			 || (get_foc_throttle_value(FOC_2) >= (Get_Spary_Limit_Value(FOC_2,MIN_SPRAY_ID) ) && get_cali_flow_freq() <= 1))
	{
		...
		{
			count_detect_1 = 500;
		  if(get_capacity_flag_1L() == 1)//¸¡×Ó´¥·¢ºóÔÙÓÃ´Ë·½·¨ÅĞ¶ÏÎŞÒ©
			{
	...
	{
		 count_detect_1 = 0;
		 if(FOC_1_STATUS.valid_spray == 1)// ·É»úÆğ·ÉºóÑ¹Á¦ÅĞ¶Ï
		 {
...
Ñ¹Á¦¼ÆÕï¶Ï£º
1.Ë®±ÃÃ»ÓĞÆô¶¯µÄÇé¿öÏÂ£¬Ñ¹Á¦´«¸ĞÆ÷ÊıÖµ´óÓÚ1200mv(Õâ¸öÑ¹Á¦ÏÂ¿Ï¶¨»áÓĞË®ÅçÈ÷³ö)
  ´«¸ĞÆ÷¹ÊÕÏ
2.ÓÍÃÅÖµ´óÓÚ×îĞ¡Öµ£¬µ«ÊÇÑ¹Á¦ÖµĞ¡ÓÚÕı³£¹¤×÷µÄ £¬¹ÜÂ·ÓĞ¿ÕÆø»òÕßÑ¹Á¦´«¸ĞÆ÷¹ÊÕÏ
3.ÓÍÃÅÖµ´óÓÚ×îĞ¡Öµ£¬µ«Ñ¹Á¦Öµ¹ı´óË®±Ã¶ÂÈû»òÕßÑ¹Á¦´«¸ĞÆ÷¹ÊÕÏ
4.ÈÎºÎÇé¿öÏÂÑ¹Á¦´«¸ĞÆ÷ÊıÖµĞ¡ÓÚ400mv,»òÕß´óÓÚ4000mvÖ±½Ó±¨¹ÊÕÏ

Á÷Á¿¼ÆÕï¶Ï£º
1.Ë®±ÃÃ»ÓĞ¹¤×÷£¬Á÷Á¿¼ÆÓĞ¼ÆÊı£¬Á÷Á¿¼Æ´íÎó£¨Ã»ÓĞÓöµ½¹ı£¬ÏÈ²»¼Ó£©
2.Ë®±Ã¹¤×÷£¬µ«ÊÇÃ»ÓĞÂö³åÊı£¬Ö±½Ó»á±¨ÎŞÒ©
3.Âö³åÊı¹¤×÷·¶Î§ÔİÊ±Ã»¼û¹ıÌ«Òì³£µÄ£¨ºóĞø²¹³ä£©
